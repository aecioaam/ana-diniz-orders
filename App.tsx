...
const App: React.FC = () => {
  // Inicializa sempre como falso para evitar acesso direto via URL/Reload
  ...
  // Limpa o hash no carregamento para garantir que comece na loja
  useEffect(() => {
    ...
    <div className="max-w-xl mx-auto min-h-screen bg-[#FDF4F4] shadow-2xl flex flex-col font-sans relative">
      <header className="bg-[#D4A3A3] text-white p-6 sticky top-0 z-50 flex justify-between items-center rounded-b-[2.5rem] shadow-lg border-b border-white/20">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black tracking-tight leading-none">Ana Diniz</h1>
          ...
          <>
                    ...
                    <div key={product.id} className="flex gap-4 p-4 bg-white rounded-[2rem] border border-[#F2E4E4] shadow-sm active:scale-95 transition-all group overflow-hidden">
                      ...
                      <div className="flex-1 flex flex-col justify-between py-1">
                        ...
                        <div className="flex justify-between items-center mt-3">
                          ...
                          <button onClick={() => product.options ? setSelectedProductForOptions(product) : handleAddToCart(product)} className="bg-[#D4A3A3] text-white p-3 rounded-2xl shadow-lg active:scale-90 transition hover:bg-[#C28E8E]">
                            {product.options ? <ChevronDown size={20} /> : <Plus size={20} />}
                          </button>
                        </div>
                      </div>
                    </div>
            ...
            {step === 4 && (
              <div className="p-6 space-y-8 animate-in slide-in-from-right-4 pb-12">
                {/* Resumo Final movido para o topo da etapa 4 */}
                <section className="space-y-4">
                ...
                {/* Pagamento agora abaixo do resumo */}
                <section className="space-y-4">
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-md p-6">
          <div className="w-full max-sm bg-white rounded-[3rem] p-8 space-y-8 shadow-2xl animate-in zoom-in duration-300">
            <div className="text-center space-y-2">
              ...
              <p className="text-sm text-[#8C6B6B] font-medium italic">Digite a senha para acessar o painel</p>
            </div>
            <div className="space-y-4">
              <input 
                type="password" 
                ...
                autoFocus
                className="w-full border-2 border-[#F2E4E4] p-5 rounded-[2rem] text-center text-xl font-black focus:border-[#D4A3A3] outline-none transition-all shadow-inner"
